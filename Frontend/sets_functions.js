function list_products_onload(){
    fetch("/list_sets", {method: 'POST'}).then((res) =>{
        ...
        for(let i = 0; i< data.length;i++){
            ...
            cell1.setAttribute("name","setID");
            let cell2 = row.insertCell();
            cell2.setAttribute("name","setname");
            let cell3 = row.insertCell();
            cell3.setAttribute("name","setprice");
            let cell4 = row.insertCell();
            cell4.setAttribute("name","setpicture");
...
function addToCart(r){
    ...
    let id = document.getElementById("setstable").rows[i].cells[0].innerHTML;
    let name = document.getElementById("setstable").rows[i].cells[1].innerHTML;
    localStorage.setItem(id, window[name]++);
    document.getElementById("setstable").rows[i].cells[6].innerHTML = window[name];
}
...
function add_to_wish(r){
    ...
    wishlist_items.push(JSON.stringify(document.getElementById("setstable").rows[i].cells[1].innerHTML));
    wishlist_items.push(JSON.stringify("|"));
    wishlist_items.push(JSON.stringify(document.getElementById("setstable").rows[i].cells[2].innerHTML));
    wishlist_items.push(JSON.stringify("|"));
